...
export function calculateForecast(leads: Lead[]) {
    ...
    for (const lead of leads) {
         ...
         * =========================
         */
        const probability = Math.max(0, Math.min(100, Number(score.probability || 0)));
        ...
        if (!Number.isNaN(date.getTime())) {
            ...
            monthlyMap.set(month, (monthlyMap.get(month) || 0) +
                weighted);
        }
    }
    /*
     ...
     */
    const averageHealth = activeDeals > 0
        ? Math.round(totalHealth / activeDeals)
        : 0;
    const averageProbability = activeDeals > 0
     ...
     */
    const confidence = pipelineValue > 0
        ? Math.round((weightedRevenue /
            pipelineValue) *
            100)
        : 0;
    /*
   ...
   */
    ...
    const largestActiveDeal = activeDealValues.length > 0
