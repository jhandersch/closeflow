...
export async function createRouteSupabase(request: RouteRequest) {
    ...
    return createSupabaseClient(supabaseUrl, getSupabasePublicKey(), {
        global: {
            headers: {
...
const decodeJwtPayload = (token: string): Record<string, unknown> | null => {
    ...
    try {
        const payload = parts[1]
            .replace(/-/g, "+")
            .replace(/_/g, "/");
        const padded = payload.padEnd(payload.length +
...
export async function getRequestAal(request: RouteRequest, supabase: Awaited<ReturnType<typeof createRouteSupabase>>): Promise<"aal1" | "aal2"> {
    const authorization = request.headers.get("authorization") ??
        request.headers.get("Authorization");
    const bearerToken = authorization?.startsWith("Bearer ")
        ? authorization.slice(7)
...
export async function requireMfaForWorkspaceRole(request: RouteRequest, supabase: Awaited<ReturnType<typeof createRouteSupabase>>, workspaceId: string, userId: string, privilegedRoles: string[] = [
    "owner",
    "admin"
...
export async function requireAal2(request: RouteRequest, supabase: Awaited<ReturnType<typeof createRouteSupabase>>): Promise<RouteAuthzResult> {
    const aal = await getRequestAal(request, supabase);
    if (aal !== "aal2") {
        return {
            ok: false,
...
export const slugifyWorkspaceName = (value: string) => value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
