...
import type { ActivityType, Lead, LeadSource, LeadStatus, } from "@/types";
...
export function useLeadActions(onSuccess?: (newStatus: LeadStatus) => Promise<void>) {
    ...
    async function saveLead(leadId: string, oldStatus: LeadStatus, data: UpdateLeadData): Promise<Lead> {
    ...
    async function deleteLead(leadId: string) {
        ...
        if (!response.ok) {
            ...
            throw new Error(body?.error || "Lead deletion failed");
        }
    }
    return {
        saveLead,
