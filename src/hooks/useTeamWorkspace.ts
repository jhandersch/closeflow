...
const slugify = (value: string) => value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
const isSchemaError = (message?: string) => /relation .* does not exist|column .* does not exist|function .* does not exist|could not find the function/i.test(message || "");
...
export function useTeamWorkspace(inviteToken?: string | null) {
    ...
    const normalizeFromDatabase = (organization: DbOrganization, members: DbMemberRow[], invites: DbInviteRow[]): TeamWorkspace => {
    ...
    const ensureDatabaseWorkspace = async (userId: string, email: string, displayName: string, metadata: UserMetadata, token?: string | null) => {
        ...
        setSource("database");
        return normalizeFromDatabase(organization as DbOrganization, (members || []) as DbMemberRow[], (invites || []) as DbInviteRow[]);
    };
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (databaseWorkspace) {
                ...
                setWorkspace(includesCurrentUser
                    ? metadataWorkspace
                    : {
                        ...metadataWorkspace,
    ...
    const updateOrganization = async (name: string) => {
        if (!workspace)
            return { ok: false as const, message: "Workspace not loaded" };
        const trimmed = name.trim();
        if (!trimmed)
            return { ok: false as const, message: "Organization name is required" };
