...
export default function TaskBoard({
  ...
  const renderEditForm = (task: Task) => {
    ...
    return (
      <div className="mt-3 space-y-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3">
        <input
          value={editTitle}
          onChange={(event) =>
          ...
          className="w-full rounded-lg border border-border-subtle bg-surface-2 px-3 py-2 text-sm text-foreground outline-none"
        />
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          >
            <option value="low">
              {resolvedIsDe ? "Niedrig" : "Low"}
            </option>

            <option value="medium">
              {resolvedIsDe ? "Mittel" : "Medium"}
            </option>

            <option value="high">
              {resolvedIsDe ? "Hoch" : "High"}
            </option>

            <option value="urgent">
              {resolvedIsDe ? "Dringend" : "Urgent"}
            </option>
          </select>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() =>
              void saveEditing(task.id)
            }
            className="rounded-lg bg-foreground px-3 py-1 text-xs font-semibold text-background disabled:opacity-60"
          >
          ...
          <button
            ...
            className="rounded-lg border border-border-subtle px-3 py-1 text-xs font-semibold text-foreground/70 disabled:opacity-60"
          >
            {resolvedIsDe
              ? "Abbrechen"
              : "Cancel"}
          ...
          {open.length === 0 ? (
            ...
            open.map((task) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-foreground/55">
                  {task.due_date
                    ? `${resolvedIsDe ? "Faellig" : "Due"}: ${new Date(
                        task.due_date
