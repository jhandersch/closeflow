...
export function EmailComposeModal({ leadId, defaultTo = "", onClose, onSent }: EmailComposeModalProps) {
  ...
  const handleSend = async () => {
    ...
    if (!trimmedTo || !trimmedSubject || !trimmedBody) {
      toast.error(isDe ? "Bitte fülle alle Felder aus, bevor du sendest." : "Fill in all fields before sending.")
      return
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if (!emailRegex.test(trimmedTo)) {
      toast.error(isDe ? "Bitte gib eine gültige E-Mail-Adresse ein." : "Enter a valid email address.")
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(data.error || (isDe ? "E-Mail konnte nicht protokolliert werden." : "Could not log email."))
        return
      }

      toast.success(isDe ? `E-Mail protokolliert: "${trimmedSubject}"` : `Email logged: "${trimmedSubject}"`)
      onSent?.()
      onClose()
    } catch {
      toast.error(isDe ? "E-Mail konnte nicht protokolliert werden." : "Failed to log email.")
    } finally {
      setSending(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-border-subtle bg-surface-1 p-6 shadow-2xl">
        <div className="mb-5 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center rounded-xl border border-border-subtle text-foreground/60 transition hover:bg-foreground/5 hover:text-foreground"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-xs uppercase tracking-[0.2em] text-foreground/50">{isDe ? "An" : "To"}</label>
            <input
              type="email"
              value={to}
              onChange={(e) => setTo(e.target.value)}
              placeholder={isDe ? "kontakt@beispiel.de" : "contact@example.com"}
              className="w-full rounded-xl border border-border-subtle bg-surface-2 px-4 py-2.5 text-sm text-foreground outline-none placeholder:text-foreground/35 focus:border-cyan-500/50"
            />
          </div>
          <div>
            ...
            <input
              ...
              placeholder={isDe ? "Rückfrage zu unserem Gespräch" : "Following up on our conversation"}
              className="w-full rounded-xl border border-border-subtle bg-surface-2 px-4 py-2.5 text-sm text-foreground outline-none placeholder:text-foreground/35 focus:border-cyan-500/50"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-[0.2em] text-foreground/50">{isDe ? "Nachricht" : "Message"}</label>
            <textarea
              value={body}
