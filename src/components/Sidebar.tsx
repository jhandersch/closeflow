...
export default function Sidebar() {
    const { canManageWorkspace, canManageBilling, isPlatformAdmin, } = usePermissions();
    ...
    return (<>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col overflow-y-auto border-r border-border-subtle bg-gradient-to-b from-surface-1 to-surface-2 px-5 py-6 shadow-2xl transition-transform duration-200 lg:hidden ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <SidebarContent hydrated={hydrated} links={links} t={t} pathname={pathname} notificationCount={notificationCount} onLinkClick={() => setOpen(false)} onLogout={() => void handleLogout()}/>
      </aside>
    </>);
}
type LinkDef = {
...
function SidebarContent({ hydrated, links, t, pathname, notificationCount, onLinkClick, onLogout, }: {
    hydrated: boolean;
    links: LinkDef[];
      ...
      <nav className="flex-1 space-y-1.5">
        {links.map((link) => {
            ...
            return (<Link key={link.href} href={link.href} onClick={onLinkClick} className={`flex items-center gap-3 rounded-2xl px-4 py-2.5 text-sm font-medium transition ${active
              ...
              <Icon size={17}/>
              {hydrated ? (<span className="flex-1">{link.label}</span>) : (<span className="flex-1">
                  <span className="block h-3 w-20 animate-pulse rounded-full bg-foreground/10"/>
                </span>)}
              {isSearch ? (<span className="rounded-md border border-border-subtle px-1.5 py-0.5 text-[10px] text-foreground/55">
                  Ctrl+K
                </span>) : null}
              {isNotifications && notificationCount > 0 ? (<span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-rose-500/80 px-1.5 text-[10px] font-semibold text-white">
