...
export default function RevenueForecastAI({ insight = null, loading = false, error = null, }: Props) {
    if (error) {
        return (<section className="rounded-3xl border border-border-subtle bg-surface-1 p-6">
    ...
    const healthColor = insight.health === "Excellent"
    ...
    const healthLabel = insight.health;
    return (<section className="
        rounded-2xl
        border
        border-cyan-500/20
      ...
      <div className="
          ...
          lg:grid-cols-2
        ">
        <InsightList title={"Revenue Drivers"} items={insight.topDrivers} color="emerald" emptyText={"No drivers detected."}/>

        <InsightList title={"Risks"} items={insight.risks} color="amber" emptyText={"No risks detected."}/>
      </div>
      ...
      <div className="
        ...
        <ul className="mt-3 space-y-2">
          {insight.recommendations.length > 0 ? (insight.recommendations.map((item, index) => (<li key={`${item}-${index}`} className="
                  rounded-lg
                  border
                  border-border-subtle
                  bg-surface-1
                ...
                ">
                → {item}
              </li>))) : (<li className="text-sm text-foreground/60">
...
function InsightList({ title, items, color, emptyText, }: {
    title: string;
    items: string[];
    color: "emerald" | "amber";
    emptyText: string;
    ...
    return (<div>
      ...
      <ul className="mt-3 space-y-2">
        {items.length > 0 ? (items.map((item, index) => (<li key={`${item}-${index}`} className={`
                rounded-lg
                border
                px-3
                py-2
