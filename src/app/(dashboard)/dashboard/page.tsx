...
export default function DashboardPage() {
    ...
    const { analysis: forecastAnalysis, loading: forecastLoading, } = useForecastAI(forecast, leads, language);
    const { insight: revenueInsight, loading: revenueInsightLoading, error: revenueInsightError, } = useRevenueForecastAI(leads, forecast, language);
    ...
    const activityTrendData = useMemo(() => {
        ...
        const labels = weekStarts.map((date) => date.toLocaleDateString("en-US", {
            day: "2-digit",
            month: "2-digit",
        }));
    ...
    if (loading) {
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            {Array.from({
                length: 4
            }).map((_, i) => (<div key={i} className="h-28 animate-pulse rounded-3xl border border-border-subtle bg-surface-1"/>))}
    ...
    if (error) {
        ...
        ">
          ...
          <button onClick={() => void refresh()} className="
              mt-5
              rounded-xl
              border
              border-rose-400/30
    ...
    if (!leads.length) {
        ...
        <div className="space-y-6">
          ...
          <EmptyState icon={<span className="
                rounded-full
                border
                border-cyan-500/30
                bg-cyan-500/10
    ...
    return (<AuthGuard>
      ...
      <div className="space-y-8">
        ...
        <DashboardHeader forecast={metrics.forecast} totalLeads={metrics.total} pipelineValue={metrics.pipelineValue} attentionCount={metrics.atRiskDeals.length}/>
        ...
        <KPIGrid totalLeads={metrics.total} pipelineValue={metrics.pipelineValue} wonDeals={metrics.won} revenue={metrics.revenue} conversionRate={metrics.conversionRate} activitiesThisWeek={activitiesThisWeek} openTasks={taskSummary.open}/>
        ...
        <AIForecastCard analysis={forecastAnalysis} loading={forecastLoading}/>
        ...
        <RevenueForecastAI insight={revenueInsight} loading={revenueInsightLoading} error={revenueInsightError}/>
        ...
        ">
          ...
          <div className="space-y-6">
            ...
            <TasksWidget open={taskSummary.open} completed={taskSummary.completed} overdue={taskSummary.overdue} highPriorityOpen={taskSummary.highPriorityOpen} nextDue={taskSummary.nextDue} loading={tasksLoading}/>
