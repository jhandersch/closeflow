...
export default function CustomersPage() {
    ...
    useEffect(() => {
        return () => {
            if (importMessageTimeoutRef.current) {
                window.clearTimeout(importMessageTimeoutRef.current);
            }
        };
    }, []);
    const customers = useMemo(() => {
        ...
        for (const lead of leads) {
            ...
            if (!existing) {
                byCustomer.set(key, {
                    ...
                    totalRevenue: lead.status === "won"
                        ? lead.value || 0
                        : 0,
                    deals: 1,
                    wonDeals: lead.status === "won"
                        ? 1
                        : 0,
                    lostDeals: lead.status === "lost"
                        ? 1
                        : 0,
        ...
        return Array.from(byCustomer.values()).sort((a, b) => b.totalRevenue - a.totalRevenue);
    }, [leads]);
    ...
    const importCsv = async (file: File) => {
        ...
        try {
            ...
            const summary = `${t("customers.importDoneStart", "Import done. Added")} ` +
                `${inserted} ` +
    ...
    return (<div className="mx-auto max-w-6xl space-y-6">
        <div>
          <p className="text-sm uppercase tracking-[0.24em] text-cyan-400">
            {t("customers.header", "Customers")}
          </p>

          <h1 className="mt-2 text-3xl font-bold text-foreground">
            {t("customers.title", "Customersverwaltung")}
          </h1>

          <p className="mt-2 text-sm text-foreground/65">
            {t("customers.subtitle", "Verwalte aktive Customers, Abwanderungsrisiken und wichtige VIP-Customers.")}
          </p>
        </div>

        <div className="grid gap-3 md:grid-cols-[1fr_auto_auto_auto_auto_auto_auto]">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("customers.searchPlaceholder", "Customers suchen...")} className="w-full rounded-xl border border-border-subtle bg-surface-1 px-4 py-2 text-sm text-foreground outline-none"/>

          <button onClick={() => {
            setFilter("all");
            setCustomerType("all");
        }} className={`rounded-xl border px-4 py-2 text-sm ${filter === "all" && customerType === "all"
            ? "border-foreground bg-foreground text-background"
            : "border-border-subtle text-foreground/80"}`}>
            {t("customers.filterAll", "All")}
          </button>

          <button onClick={() => setFilter((current) => current === "active" ? "all" : "active")} className={`rounded-xl border px-4 py-2 text-sm ${filter === "active"
            ? "border-foreground bg-foreground text-background"
            : "border-border-subtle text-foreground/80"}`}>
            {t("customers.filterActive", "Active Customers")}
          </button>

          <button onClick={() => setFilter((current) => current === "lost" ? "all" : "lost")} className={`rounded-xl border px-4 py-2 text-sm ${filter === "lost"
            ? "border-foreground bg-foreground text-background"
            : "border-border-subtle text-foreground/80"}`}>
            {t("customers.filterLost", "Lost Customers")}
          </button>

          <button onClick={() => setFilter((current) => current === "vip" ? "all" : "vip")} className={`rounded-xl border px-4 py-2 text-sm ${filter === "vip"
            ? "border-foreground bg-foreground text-background"
            : "border-border-subtle text-foreground/80"}`}>
            VIP
          </button>

          <button onClick={() => setCustomerType((current) => current === "companies" ? "all" : "companies")} className={`rounded-xl border px-4 py-2 text-sm ${customerType === "companies"
            ? "border-foreground bg-foreground text-background"
            : "border-border-subtle text-foreground/80"}`}>
            Companies
          </button>

          <button onClick={() => setCustomerType((current) => current === "private" ? "all" : "private")} className={`rounded-xl border px-4 py-2 text-sm ${customerType === "private"
            ? "border-foreground bg-foreground text-background"
            : "border-border-subtle text-foreground/80"}`}>
            Private Customers
          </button>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={() => void downloadExport("csv")} className="rounded-xl border border-border-subtle bg-surface-1 px-4 py-2 text-sm text-foreground/80 hover:bg-foreground/5">
            {t("customers.exportCsv", "Export CSV")}
          </button>

          <button onClick={() => void downloadExport("xlsx")} className="rounded-xl border border-border-subtle bg-surface-1 px-4 py-2 text-sm text-foreground/80 hover:bg-foreground/5">
            {t("customers.exportExcel", "Export Excel")}
          </button>

          <button onClick={() => fileInputRef.current?.click()} disabled={importingCsv} className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-sm font-semibold text-cyan-300 disabled:opacity-60">
            {importingCsv
            ? t("customers.importing", "Importing...")
            : t("customers.importFile", "Datei importieren")}
          </button>
          ...
          {importIssues.length ? (<button onClick={downloadImportIssuesReport} className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-2 text-sm font-semibold text-amber-200">
              {t("customers.downloadImportIssues", "Download Import Issues")}
            </button>) : null}

          {importMessage ? (<div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-2.5 text-sm text-emerald-300">
              {importMessage}
            </div>) : null}

          {importError ? (<div className="rounded-xl border border-rose-500/20 bg-rose-500/10 px-4 py-2.5 text-sm text-rose-300">
              {importError}
            </div>) : null}
        </div>

        {error ? (<div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4 text-sm text-rose-200">
            {t("customers.loadErrorPrefix", "Could not load customers:")}{" "}
            {error}
          </div>) : null}

        {loading && customers.length === 0 ? (<div className="rounded-2xl border border-border-subtle bg-surface-1 p-6 text-foreground">
            {t("customers.loading", "Loading...")}
          </div>) : filteredCustomers.length === 0 ? (customers.length === 0 ? (<div className="rounded-2xl border border-border-subtle bg-surface-1 p-10 text-center">
              ...
              <h3 className="mt-4 text-xl font-semibold text-foreground">
                {t("customers.noCustomersYet", "No customers yet")}
              </h3>

              <p className="mt-2 text-sm text-foreground/65">
                {t("customers.noCustomersYetDescription", "Customers will appear here once your deals are completed.")}
              </p>
            </div>) : (<div className="rounded-2xl border border-border-subtle bg-surface-1 p-10 text-center">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-cyan-500/30 bg-cyan-500/10 text-cyan-300">
                ×
              </div>
              ...
              <p className="mt-2 text-sm text-foreground/65">
                {t("customers.noResultsDescription", "No customers match your current search or filter.")}
              </p>
            ...
            {(["Companies", "Private Customers"] as const).map((section) => {
                const sectionCustomers = filteredCustomers.filter((customer) => section === "Private Customers"
                    ? customer.isPrivate
                    : !customer.isPrivate);
                ...
                return (<section key={section} className="space-y-3">
                  ...
                  {sectionCustomers.map((customer) => (<div key={customer.id} className="rounded-2xl border border-border-subtle bg-surface-1 p-5 transition hover:border-cyan-400/30">
                <div className="flex items-center justify-between gap-4">
                  <Link href={`/customers/${encodeURIComponent(customer.id)}`} className="min-w-0 flex-1">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        {customer.isVip ? (<span className="mt-2 inline-flex rounded-full border border-amber-400/30 bg-amber-400/10 px-2.5 py-1 text-xs font-semibold text-amber-300">
                            ★ VIP
                          </span>) : null}

                        <p className="mt-1 text-sm text-foreground/65">
                          {t("customers.contacts", "Contacts")}
                          :{" "}
                      ...
                      <div className="grid gap-3 text-sm md:grid-cols-3">
                        ...
                        <div>
                          <p className="text-foreground/55">
                            {t("customers.lastContact", "Last Contact")}
                          </p>

                          <p className="font-semibold text-foreground">
                            {customer.lastContactAt
                            ? new Date(customer.lastContactAt).toLocaleDateString(locale)
                            : t("customers.na", "n/a")}
                          </p>
                        </div>
                      </div>
                  ...
                  <button type="button" onClick={() => void deleteCustomer(customer)} disabled={deletingCustomer ===
                            customer.id} className="shrink-0 rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm font-semibold text-rose-300 transition hover:bg-rose-500/20 disabled:cursor-not-allowed disabled:opacity-50">
                    {deletingCustomer ===
                            customer.id
                            ? t("customers.deleting", "Deleting...")
