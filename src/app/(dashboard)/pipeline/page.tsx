...
type StageKey = "new" | "contacted" | "proposal" | "won"
    ...
    <AuthGuard>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-foreground">Kanban Pipeline</h1>
          <p className="mt-2 text-sm text-foreground/65">Total pipeline: €{totalValue.toLocaleString("de-DE")}</p>
        </div>
                        ...
                        {columns[stage.key].map((lead, index) => (
                          <Draggable key={lead.id} draggableId={lead.id} index={index}>
                            {(draggableProvided) => (
                              ...
                              >
                                <p className="font-semibold text-foreground">{lead.name || "Untitled lead"}</p>
                                <p className="mt-1 text-xs text-foreground/55">{lead.company || "No company"}</p>
