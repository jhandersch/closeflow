...
export default function PricingPage() {
    ...
    const startCheckout = async (plan: PlanKey) => {
        ...
        if (!response.ok) {
            setError(payload?.error || payload?.message || ("Could not start checkout."));
            setPendingPlan(null);
            return;
        }
        if (!payload?.checkoutUrl) {
            setError(payload?.message || ("Stripe checkout is not configured yet."));
            setPendingPlan(null);
            return;
        }
    ...
    return (<AuthGuard>
      <div className="mx-auto max-w-6xl space-y-8">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-foreground">{"Choose your growth plan"}</h1>
          ...
          <p className="mt-2 text-xs text-foreground/55">
            {"Current plan"}: {billingLoading ? ("Loading...") : (billing?.plan || "free").toUpperCase()} ({billing?.status || ("inactive")})
          </p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {plans.map((plan) => {
            ...
            return (<PlanCard key={plan.key} name={plan.name} price={plan.price} subtitle={plan.subtitle} description={plan.description} features={plan.features} featured={plan.featured} active={active} ctaLabel={active
                    ? ("Current plan")
                    : loading
                        ? ("Starting checkout...")
                        :
                            plan.cta} disabled={active || loading || pendingPlan !== null || !plan.checkout} onClick={plan.checkout
                    ? () => {
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[640px] border-collapse text-sm">
              ...
              <tbody className="text-foreground/85">
                ...
                <tr>
                  <td className="py-3 pr-4">{"Automation volume"}</td>
...
function PlanCard({ name, price, subtitle, description, features, ctaLabel, featured, active, disabled, onClick, }: {
    name: string;
    price: string;
    subtitle: string;
    description: string;
    ...
    return (<div className={`rounded-2xl border p-6 ${featured ? "border-cyan-500/30 bg-cyan-500/10" : "border-border-subtle bg-surface-1"}`}>
      ...
      <button type="button" onClick={onClick} disabled={disabled} className={`mt-6 inline-flex rounded-xl px-4 py-2 font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${active ? "border border-emerald-500/30 bg-emerald-500/10 text-emerald-200" : "bg-white text-black hover:opacity-90"}`}>
        {ctaLabel}
      </button>
    </div>);
}
