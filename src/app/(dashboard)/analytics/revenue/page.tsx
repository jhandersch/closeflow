...
export default function RevenueAnalyticsPage() {
    ...
    const [revenueEvents, setRevenueEvents] = useState<Array<{
        month: string;
        value: number;
    }>>([]);
    useEffect(() => {
        const load = async () => {
            ...
            if (!workspaceId) {
                ...
                if (!workspacesResponse.ok)
                    return;
                const bundles = (await workspacesResponse.json()) as Array<{
                    workspace: {
                        id: string;
                    };
                }>;
    ...
    return (<AuthGuard>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Card label={"Revenue"} value={`€${metrics.revenue.toLocaleString(locale)}`}/>
          <Card label={"Average deal size"} value={`€${metrics.averageDealValue.toLocaleString(locale)}`}/>
