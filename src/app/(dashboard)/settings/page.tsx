...
export default function SettingsPage() {
    ...
    const { language: appLanguage, theme: appTheme, setLanguage: setAppLanguage, setTheme: setAppTheme, t, } = useAppPreferences();
    const locale = "en-US";
    ...
    useEffect(() => {
        const loadUser = async () => {
            ...
            setMfaAvailable(Boolean((supabase.auth as unknown as {
                mfa?: unknown;
            }).mfa));
            await syncMfaState();
    ...
    const deleteAccount = async () => {
        ...
        if (!response.ok) {
            ...
            toast.error(text || ("Could not delete account"));
    ...
    const elevateMfaSession = async () => {
        ...
        if (!mfaApi?.challenge || !mfaApi?.verify || !mfaFactorId) {
            toast.error("Could not start MFA session.");
            return;
        }
        ...
        setMfaBusy(true);
        try {
            const { data: challengeData, error: challengeError } = await mfaApi.challenge({
                factorId: mfaFactorId,
            });
            if (challengeError) {
            ...
            toast.success("MFA session verified. AAL2 is now active.");
        }
        catch (error) {
    ...
    const disableMfa = async () => {
        ...
        setMfaBusy(true);
        try {
            const { data: aal, error: aalError } = await mfaApi.getAuthenticatorAssuranceLevel();
            if (aalError) {
            ...
            if (aal.currentLevel !== "aal2") {
                toast.error("Please verify your 2FA with your authenticator code first.");
                return;
            }
