...
const selectClause = `
  ...
  updated_at
`;
const createCalendarActivity = async ({ supabase, workspaceId, userId, leadId, type, title, description, metadata, }: {
...
export async function POST(request: Request) {
    ...
    const scheduledAt = typeof body.scheduled_at === "string" && isValidIso(body.scheduled_at)
        ? body.scheduled_at
        : new Date().toISOString();
    const leadId = typeof body.lead_id === "string" && body.lead_id.trim()
        ? body.lead_id.trim()
        : null;
    const status = typeof body.status === "string" && allowedStatuses.has(body.status.trim())
        ? body.status.trim()
        : "scheduled";
    if (!title) {
...
export async function PUT(request: Request) {
    ...
    const scheduledAt = typeof body.scheduled_at === "string" && isValidIso(body.scheduled_at)
        ? body.scheduled_at
        : undefined;
    const leadId = typeof body.lead_id === "string" ? body.lead_id.trim() || null : undefined;
    const status = typeof body.status === "string" && allowedStatuses.has(body.status.trim())
        ? body.status.trim()
        : body.status === null
            ? null
