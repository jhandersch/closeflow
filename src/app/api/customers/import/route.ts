...
export async function POST(request: Request) {
  ...
  if (lines.length < 2) {
    return NextResponse.json({ inserted: 0, skipped: 0, message: "No rows found", issues: [] })
  }
  ...
  const knownKeys = new Set(
    (existingLeads || []).map(
      (lead) => `${(lead.name || "").trim().toLowerCase()}::${(lead.company || "").trim().toLowerCase()}`
    )
  )
  ...
  for (const [lineIndex, line] of lines.slice(1).entries()) {
    ...
    const contact = get("contact") || `Contact ${rowNumber - 1}`
    ...
    const key = `${contact.trim().toLowerCase()}::${company.trim().toLowerCase()}`
