...
export async function POST(request: Request) {
    ...
    const { workspace, error: workspaceError } =
        await loadWorkspaceForUser(supabase, user.id);
    ...
    const returnUrl =
        ...
        `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/settings/billing`;
    ...
    try {
        ...
        return NextResponse.json(
            { error: "Could not create Stripe billing portal session" },
            { status: 500 },
        );
    }
}