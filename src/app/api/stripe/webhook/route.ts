  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (workspaceId) {
        ...
        if (stripeSubscriptionId) {
          ...
          const subscriptionPeriodEnd = Number((stripeSubscription as unknown as { current_period_end?: number }).current_period_end || 0)
    ...
    if (
      ...
      const updatePayload = {
        ...
        current_period_end: Number((subscription as unknown as { current_period_end?: number }).current_period_end || 0)
          ? new Date(Number((subscription as unknown as { current_period_end?: number }).current_period_end) * 1000).toISOString()
          : null,
        updated_at: new Date().toISOString(),
      }
