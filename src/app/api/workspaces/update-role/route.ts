...
export async function POST(request: Request) {
    ...
    * change roles.
    ...
    if (workspaceRole.role !== "owner" &&
        ...
        return NextResponse.json({
            error: "Only workspace owners and admins can change member roles.",
        }, { status: 403 });
    }
    /*
    ...
    * The owner role cannot be changed.
    ...
    /*
    * Only the owner may assign or manage
    * administrator roles.
     */
    if (workspaceRole.role === "admin" &&
        (targetMember.role === "admin" ||
            ...
            role === "owner")) {
        return NextResponse.json({
            error: "Admins cannot manage admin roles.",
        }, { status: 403 });
    }
    /*
    * No one may create a second owner
    * through this API.
     */
    if (role === "owner") {
        return NextResponse.json({
            error: "The workspace owner role cannot be assigned.",
        }, { status: 403 });
    }
    /*
    * Update the role.
